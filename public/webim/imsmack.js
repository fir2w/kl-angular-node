...
function smackAdd(userName, keyUrl, time) {
  ...
  // ------------------------------------好友列表----------------------------------------
  // 最小化
  _this.find("a#wbim_icon_mini").unbind("click").bind("click", function () {
    var iform = _this.find("div#wbim_list_expand");
    iform.hide();
  });
  _this.find("div#wbim_list_pos").unbind("click").bind("click", function () {
    var iform = _this.find("div#wbim_list_expand");
    iform.hide();
  });
  // 最大化
  _this.find("div#wbim_min_friend").unbind("click").bind("click", function () {
    var iform = _this.find("div#wbim_list_expand");
    iform.show();
  });
  _this.find("div#wbim_min_friend").hover(function () {
    $(this).addClass("wbim_min_box_hover");
  }, function () {
    $(this).removeClass("wbim_min_box_hover");
  });
  // 状态设置鼠标移入移出效果
  _this.find("div#wbim_tit_lf").hover(
    // 移入
    function () {
      $(this).find("ul").show();
  ...
  // 状态设置
  _this.find("div#wbim_tit_lf").find("li").each(function () {
    var o = $(this);
    o.unbind("click").bind("click", function () {
      ...
      function callback() {
        _this.find("div#tit").html("<span class='" + className
          + "'></span><span class='txt'>" + s_val
          + "</span><span class='icon'></span>")
        _this.find("div#wbim_min_friend").find("p").find("span").attr(
          "class", className);
        // 关闭选择页面
        _this.find("div#wbim_tit_lf").find("ul").hide();
        _this.find("div#wbim_tit_lf").removeClass("wbim_tit_lf_hover");
      }
  ...
  // 对话框关闭
  _this.find("a#rt_wbim_icon_close").unbind("click").bind("click",
    function () {
      _this.find("div#wbim_chat_box").hide();
      _this.find("div#wbim_min_chat").hide();
      _this.find("div#wbim_min_box_col_change").attr("class",
        "wbim_min_box_col2");
  ...
  // 对话框最大化
  _this.find("div#wbim_min_chat").unbind("click").bind("click", function () {
    _this.find("span#span_wbim_min_nick").text("");
    _this.find("div#wbim_chat_box").show();

    //定位快捷图标
    $("div#shortcut_div").css({"position": "absolute", "right": "421px", "bottom": "0px", "z-index": "1001"});

    var id = _this.find("ul#wbim_chat_friend_list").find("li.wbim_active").attr("uid");
    _this.find("dl[uid='" + id + "']").parent().scrollTop(99999999);
    return false;
  });

  // 左侧列表用户点击效果
  _this.find("ul#wbim_chat_friend_list").delegate("li", "click", function () {
    var o = $(this);
    // 先改变样式
    _this.find("#wbim_chat_friend_list").find("li").each(function () {
      $(this).removeClass("wbim_active");
      // $(this).attr("class","wbim_offline");
    });
    o.removeClass("wbim_highlight");
    o.attr("class", "wbim_active");
    // 对话框上面名字显示
    _this.find("a#wbim_tit_lf_name").attr("title", o.attr("title"));
    _this.find("a#wbim_tit_lf_name").text(o.attr("title"));
    _this.find("span#wbim_tit_lf_state_class").attr("class",
      o.find("span").attr("class"));
    // 最小化的名字显示
    _this.find("div#wbim_min_chat").find("span#span_wbim_min_nick").text(o
      .attr("title"));
    // 中间的聊天内容切换
    _this.find("div#wbim_chat_list").find("dl").each(function () {
      $(this).hide();
    });
    _this.find("div#wbim_chat_list")
      .find("dl[uid='" + o.attr("uid") + "']").show();
    _this.find("div#wbim_chat_list")
      .find("dl[uid='" + o.attr("uid") + "']").parent().scrollTop(99999999);
    return false;
  });

  // 删除对话框
  _this.find("ul#wbim_chat_friend_list").delegate(
    "a[name='wbim_icon_close']", "click", function () {
      var o = $(this);
      var uid = o.parent().attr("uid");
      var cid = _this.find("ul#wbim_chat_friend_list")
        .find("li.wbim_active").attr("uid");
      var size = _this.find("ul#wbim_chat_friend_list").find("li")
        .size();
      // 如果只有一个对话人
      if (size == 1) {
        o.parent().remove();
        _this.find("div#wbim_chat_list").find("dl[uid='" + uid
          + "']").remove();
        _this.find("a#rt_wbim_icon_close").click();
      } else {
        o.parent().remove();
        _this.find("div#wbim_chat_list").find("dl[uid='" + uid
          + "']").remove();
        if (uid == cid) {
          _this.find("ul#wbim_chat_friend_list").find("li:first")
            .click();
        }
      }
      return false;
  ...
  _this.find("#wbim_icon_face").click(function () {
  ...
  //聊天记录
  _this.find("#wbim_icon_chatdoc").click(function () {
    window.open(getJspUrl('/portal/portal', {'p': 'messageHomePage#m=message&pt=l&ptc=' + _this.find("ul#wbim_chat_friend_list")
      .find("li.wbim_active").attr("uid") + '&ptd=' + _this.find("ul#wbim_chat_friend_list")
      .find("li.wbim_active").attr("title")}));

  });

  _this.find("#wbim_chat_input_ta").blur(function () {
    setTimeout(function () {
  ...
  // 发送信息
  _this.find("#input_btn_send").click(function () {
    removeFace("wbim_chat_input_ta");
    var toName = _this.find("ul#wbim_chat_friend_list")
      .find("li.wbim_active").attr("uid");
    ...
    $("#wbim_chat_input_ta").focus();
    $("#atra_num").text("0");
    var o = $("div#wbim_chat_list").find("dl[uid='" + toName + "']");
...
//			strx = strx + "<p class='txt'>" + displayHtmlCode(sends[x]) + "</p>";
    ...
    o.append("<dd class='wbim_msgr'>" + "<div class='wbim_msgpos'>"
  ...
  $("#wbim_chat_input_ta").keyup(function (e) {
    if (e.shiftKey && e.which == 13 || e.which == 10) {
      ...
      _this.find("#input_btn_send").click();
    }
...
function getGroups(userName) {
  ...
  if (json != null) {
    ...
    _this.delegate("li", "click", function () {
      var o = $(this);
      var p = _that.find("ul#wbim_chat_friend_list");
      var q = p.find("li[uid='" + o.attr("uid") + "']");
      var m = _that.find("div#wbim_chat_list");
      // 先判断对话框是否存在
      if (q.size() == 0) {
        // 对话框左侧加入说话者
        p
          .append("<li node-type='user_root' uid='"
            ...
            + "'class='wbim_offline'>"
            + "<div class='wbim_userhead'>"
            + "<img node-type='wbim_userhead' src='upload_files/avatar/im/" + o.attr("uid") + ".jpg' onerror=\"this.src='dcp/comm/image/on_error_s.gif'\"  alt='"
            + o.attr("title")
            + "'>"
            + "<span node-type='wbim_status' class='"
            + o.find("#wbim_state_fr")
            .attr("class")
            + "'></span>"
            + "</div>"
            + "<div node-type='wbim_username' class='wbim_username'>"
            ...
            + "</li>");
        // 在新建中间对话记录的div
        m.append("<dl uid='" + o.attr("uid") + "'></dl>");
        initTalk(userName, o.attr("uid"));
        p.find("li[uid='" + o.attr("uid") + "']").click();
      } else {
        p.find("li[uid='" + o.attr("uid") + "']").show();
        p.find("li[uid='" + o.attr("uid") + "']").click();
      }
      ...
      _that.find("dl[uid='" + o.attr("uid") + "']").parent().scrollTop(99999999);
      openDialog(userName, o.attr("uid"));
      return false;
    });
