    ...
    return {
      ...
      compile: function compile(tElement, tAttrs) {
        ...
        if (_child_len > 1) {
          ...
          var weightTotal = 0;
          for (_i = 0; _i < _child_len; ++_i) {
            var s = angular.element(_childens[_i]).attr('weight-size');
            if(s){
              var fl = weightTotal;
              weightTotal+= parseInt(s);
              var fr = 100 -weightTotal;
              var area = angular.element(_childens[_i]).css(flowProperty, fl + '%').css(oppositeFlowProperty,  fr + '%');
              if (_i < _child_len - 1) {//设置滚动条
                var bar = angular.element(splitBarElem_htmlTemplate).css(flowProperty, weightTotal + '%');
                area.after(bar);
              }
            }else {
