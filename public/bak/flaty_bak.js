...
//加入动画显示
  var bellNumber = $('.flaty-nav .dropdown-toggle > .fa-bell + .badge');// > 子元素 // <span class='badge badge-important'>5</span>

  if ($(bellNumber).length > 0 && parseInt($(bellNumber).html()) > 0) {//如果数字大于0
    $('.flaty-nav .dropdown-toggle > .fa-bell').addClass('anim-swing');
  }
  bellNumber = $('.flaty-nav .dropdown-toggle > .fa-envelope + .badge');
  if ($(bellNumber).length > 0 && parseInt($(bellNumber).html()) > 0) {
    $('.flaty-nav .dropdown-toggle > .fa-envelope').addClass('anim-top-down');
  }



  $('.show-tooltip').tooltip({container: 'body', delay: {show: 500}});//提示


  $('.show-popover').popover();//弹出框
  ...
  var heightHandler = function () {
    if ($('#sidebar.sidebar-fixed').size() == 0) {
      $('#sidebar .nav').css('height', 'auto');//菜单的高度auto
      return;
    }
  ...
  $('#sidebar a.dropdown-toggle').click(function () {
    ...
    //下拉效果
    subMenus.slideToggle(400, function () {
      if ($(this).is(':hidden')) {
  ...
  //对div设置点击事件，可以隐藏sidebar
  $('#sidebar-collapse').click(function () {
    $('#sidebar').toggleClass('sidebar-collapsed');
    if ($('#sidebar').hasClass('sidebar-collapsed')) {//折叠
  ...
  if ($('#nav-horizontal')) {//不知做什么
    var navHorizontalInit = function () {
      var e = $(window).width();
      if (e > 979) {
  ...
  $('#nav-horizontal a.dropdown-toggle').click(function () {
    ...
    e.slideToggle(400, function () {//下拉效果
      if ($('#nav-horizontal.nav-xs').size() > 0) {
        if ($(this).is(':hidden')) {
          t.attr('class', 'arrow fa fa-angle-right');
        } else {
...
//设置盒子的行为，根据data-action来设置行为
  ...
  $(window).scroll(function () {
    if ($(this).scrollTop() > 100) {
      $('#btn-scrollup').fadeIn();//淡入淡出
    } else {
      $('#btn-scrollup').fadeOut();
    }
  });
  $('#btn-scrollup').click(function () {//跑到前面
